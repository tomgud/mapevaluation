...
void Fitness::assign_resources(Map* m,
        const PlayerManager& pm, bool use_astar) {
    AStar search;
    double min_length(m->get_dimx()*m->get_dimy());
    int shortest_player(-1);
    int cur_length;
    for (int i = 0; i < m->get_dimy(); ++i) {
        for (int j = 0; j < m->get_dimx(); ++j) {
            if (m->at(j, i)->get_resource().get_name()
                    != Resource::NO_RES_NAME) {
                min_length = m->get_dimx()*m->get_dimy();
                shortest_player = -1;
                vector<Player> players = pm.get_players();
                cur_length = 0;
                for (unsigned int k = 0; k < players.size(); ++k) {
                    if (!use_astar) {
                        cur_length = CoOrd::get_length(CoOrd(j, i),
                                players[k].get_start());
                ...
                if (shortest_player != -1) {
                    ...
                    cout << "Critical error. Could not find any player ";
                    cout << "closest to a resource at ";
                    cout << j << "," << i << endl;
                }
            }
        }
...
void Fitness::reset_resources(Map* m) {
    for (int i = 0; i < m->get_dimy(); ++i) {
        for (int j = 0; j < m->get_dimx(); ++j) {
            m->at(j, i).get()->reset_owner();
        }
    }
}

double Fitness::starting_positions(const Map &m,
        const PlayerManager &p) {
    ...
    // total_length is the sum of the lengths between all players in team a
    // to all players in team b for example, 4 players,
    // a1_to_b1+a1_to_b2+a2_to_b1+a2_to_b2 then the average length between
    // the teams is the sum divided by two ideal length is the diagonal
    // between the square inside the map, that  covers a fourth of the map
    // i.e. top left starts at (0+DIMX/4 ; 0+DIMY/4) bottom right is at
    // (DIMX-DIMX/4, DIMY- DIMY/4) so this square has one side as
    // a = ((DIMX-DIMX/4)-(DIMX/4)) = DIMX-DIMX/2 and the other side is
    // b = ((DIMY-DIMY/4)-(DIMY/4)) = DIMY-DIMY/2
    // then according to pythagoras we have diagonal as a^2 + b^2
    double fitness = total_length/(team_a.size()+team_b.size());
    double a = m.get_dimx()/2.0;
    double b = m.get_dimy()/2.0;
...
double Fitness::starting_positions_teammates(const Map &m,
    ...
    // ( DIMX / 4 + DIMY / 4 )
    total_length = total_length / (m.get_dimx() / 4 + m.get_dimy() / 4);
    total_length = (total_length > 1.0) ? 1 -
        ((total_length - 1.0)/total_length) : total_length;
    // Max here is 1
    return total_length;
}

double Fitness::throughput_per_player(const Map& m,
        const PlayerManager& pm) {
    // gold, food, and production per player
    int kNoOfPlayers = pm.get_no_of_players();
    double player_throughput[kNoOfPlayers];
        ...
        for (int j = 0; j < m.get_dimx(); ++j) {
            if (m.at(j, i)->get_resource().get_name()
                ...
                if (m.at(j, i)->get_owner().get_id() == -1) {
                    cout << "Owner of a resource is not set, did you ";
                    cout << "forget to call assign_resources()? " << endl;
                    return -1.0;
                }
    ...
    for (int i = 0; i < pm.get_no_of_players(); ++i) {
        cur_avg = ((player_throughput[i]/total_throughput)
                /(1.0/pm.get_no_of_players()));
        if (cur_avg > 1.0) {
            cur_avg = 1 - ((cur_avg - 1.0)/cur_avg);
        }
        avg_player_throughput += cur_avg;
        // player_throughput[i]/total_throughput;
    }
...
double Fitness::unique_resources(const Map& m,
        const PlayerManager& pm) {
    ...
    for (int i = 0; i < m.get_dimy(); ++i) {
        for (int j = 0; j < m.get_dimx(); ++j) {
            if (m.at(j, i)->get_resource().get_name()
                ...
                unique_found[m.at(j, i)->get_owner().get_id()][Resource::
                    get_index(m.at(j, i)->get_resource().get_name())] = true;
            }
        }
    }
    ...
    for (int i = 0; i < no_players; ++i)  {
        ...
        // Max value is 1
        unique_per_player += static_cast<double>(
                cur_unique_count/max_resources);
    }
...
double Fitness::strategic_resource(const Map& m,
        const PlayerManager& pm) {
    double val(0.0);
    ...
    double cur_avg;
    for (int i = 0; i < pm.get_no_of_players(); ++i) {
        cur_avg = ((strategic_count[i] / total_strategics)/
                (1.0/pm.get_no_of_players()));
        if (cur_avg > 1.0) {
            cur_avg = 1 - ((cur_avg - 1.0)/cur_avg);
        }
...
double Fitness::players_in_a_continent(const Map& m,
        const PlayerManager& pm) {
    AStar search;
    double val(0);
    vector<Player> players = pm.get_players();

    // Count how many players, each player can find
    // without going through water or mountains.
    // for N players, if each player can reach (N-1)
    // players, N^2-N is the max count
    // so, total_legs / N(N-1)
...
double Fitness::total_throughput(const Map& m) {
    double total(0.0);
    for (int i = 0; i < m.get_dimy(); ++i) {
        for (int j = 0; j < m.get_dimx(); ++j) {
