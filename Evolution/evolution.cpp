...
Evolution::Evolution(unsigned int seed, int iters, int sz, double min_ch,
        int genome_sz, int no_of_p)
    : seed(seed), iters(iters), size(sz), genome_size(genome_sz),
    no_of_players(no_of_p), min_change(min_ch), all_req_min_chg(true),
    gen_pop(0) {
    Genome cur_gen;
...
void Evolution::run() {
    ...
    double c_f1(0.0), c_f2(0.0), c_f3(0.0), c_f4(0.0), c_f5(0.0);
    double cur_val_before(0.0), cur_val_now(0.0);
    IndGrade* c_ig;
    Map* c_map;
    PlayerManager* c_pm;
    ...
    for (int i = 0; i < iters; ++i) {
        for (int j = 0; j < this->size; ++j) {
            ...
            c_ig = new IndGrade(this->gen_pop[j].get_seed(), c_f1, c_f2,
                    c_f3, c_f4, c_f5);
        ...
        if (survivors.size() == 0) {
            // errorz
            cout << "Fatal error: For some reason, I don't have any ";
            cout << "survivors to evolve/mutate further. Quitting. Bye" << endl;
            return;
        }
        this->gen_pop.clear();
        ...
        if (fabs(cur_val_now-cur_val_before) < min_change) {
                cout << "Stopping evolution since changes between evolutions ";
                cout << "are " << fabs(cur_val_now-cur_val_before);
                cout << " (" << cur_val_now << " - " << cur_val_before << ")";
                cout << " are less than the minimum change " << min_change;
                cout << endl;
                return;
        }
