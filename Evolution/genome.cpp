...
Genome::Genome()
    : seed(0), size(0), tiles(0), resources(0), features(0),
    singleton_features({}),  players_start(0) {
}

Genome::Genome(const Genome& lval)
    : seed(lval.seed), size(lval.size),
...
Genome::Genome(unsigned int seed, int size, int players)
    ...
    // DIMX and DIMY removed here, need to be supplied as parameters
    // or do it on the map ? ... TODO
    boost::random::uniform_int_distribution<> y_gen(0, 12);
    boost::random::uniform_int_distribution<> x_gen(0, 12);
    boost::random::uniform_int_distribution<> tile_gen(0,
            TileBlock::Patterns::MAX_PATTERNS);
    boost::random::uniform_int_distribution<> w_gen(4, 12);
    boost::random::uniform_int_distribution<> h_gen(4, 12);
    boost::random::uniform_int_distribution<> res_gen(0, 28);
    boost::random::uniform_int_distribution<> feat_gen(0,
            Feature::FeatureName::NO_FEAT);
...
void Genome::modify(int offset) {
    ...
    boost::variate_generator <boost::mt19937&, boost::normal_distribution<>>
        rnd(rng, boost::normal_distribution<>(0, 4));
    boost::variate_generator <boost::mt19937&, boost::normal_distribution<>>
        rnd_y(rng, boost::normal_distribution<>(0, 10.0));
    boost::variate_generator <boost::mt19937&, boost::normal_distribution<>>
        rnd_x(rng, boost::normal_distribution<>(0, 10.0));
    ...
    for (unsigned int i = 0; i < this->tiles.size(); ++i) {
        // x,y
        cx = tiles[i].get_coord().get_x() + static_cast<int>(rnd_x());
        cy = tiles[i].get_coord().get_y() + static_cast<int>(rnd_y());
        // cx = (cx + Map::_DIMX) % Map::_DIMX; // x wraps
        cy = (cy < 0) ? 0 : cy;  // Don't want a negative index
        // cy = (cy > Map::_DIMY) ? Map::_DIMY : cy;
        // Dont want to go out of the map
        tiles[i].get_coord().set_x_y(cx, cy);

        // Width changes
        ch = tiles[i].get_height() + static_cast<int>(rnd());
        cw = tiles[i].get_width() + static_cast<int>(rnd());
        ch = (ch < 2)  ? 2  : ch;
        ch = (ch > 18) ? 18 : ch;
        cw = (cw < 2)  ? 2  : cw;
        cw = (cw > 18) ? 18 : cw;
        ...
        // Pattern
        tiles[i].add_pattern(static_cast<int>(rnd()));
    }

    // Resources
    for (unsigned int i = 0; i < resources.size(); ++i) {
        cx = resources[i].get_coord().get_x() + static_cast<int>(rnd_x());
        cy = resources[i].get_coord().get_y() + static_cast<int>(rnd_y());
        // cx = (cx + Map::_DIMX) % Map::_DIMX;
        // x wraps removed, since we do this in the map
        cy = (cy < 0) ? 0 : cy;  // Don't want a negative index
        // cy = (cy > Map::_DIMY) ? Map::_DIMY : cy;
        // Dont want to go out of the map // Removed, check this in the map
        resources[i].get_coord().set_x_y(cx, cy);
        resources[i].add_res(static_cast<int>(rnd()));
    }
    ...
    for (unsigned int i = 0; i < features.size(); ++i) {
        ...
        fn = Feature::FeatureName(
                (features[i].get_feat() + static_cast<int>(rnd()))
                % Feature::FeatureName::NO_FEAT);
        while (Feature::is_singleton(fn) && singleton_features.count(fn) > 0) {
            fn = Feature::FeatureName(
                (features[i].get_feat() + static_cast<int>(rnd()))
        ...
        cx = features[i].get_coord().get_x() + static_cast<int>(rnd_x());
        cy = features[i].get_coord().get_y() + static_cast<int>(rnd_y());
        // cx = (cx + Map::_DIMX) % Map::_DIMX; // x wraps
        cy = (cy < 0) ? 0 : cy;  // Don't want a negative index
        // cy = (cy > Map::_DIMY) ? Map::_DIMY : cy;
        // Dont want to go out of the map
    ...
    // Players
    for (unsigned int i = 0; i < players_start.size(); ++i) {
        cx = players_start[i].get_x() + static_cast<int>(rnd_x());
        cy = players_start[i].get_y() + static_cast<int>(rnd_y());
        // cx = (cx + Map::_DIMX) % Map::_DIMX;  // x wraps
        cy = (cy < 0) ? 0 : cy;  // Don't want a negative index
        // cy = (cy > Map::_DIMY) ? Map::_DIMY : cy;
        // Dont want to go out of the map
...
void Genome::set_player_starts(vector<CoOrd> coords) {
    ...
    this->players_start.insert(this->players_start.begin(),
