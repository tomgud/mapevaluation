...
bool Tundra::set_resource(const Resource &res ) {
    this->res = res;
    return false;
}

bool Tundra::set_feature(const Feature &feat) {
    this->feat = feat;
    return true;
}

Resource& Tundra::get_rand_resource(const Resource::ResourceType type,
        int rnum) const {
    switch (type) {
        case Resource::Bonus:
