...
bool Desert::set_resource(const Resource &res ) {
    this->res = res;
    return true;
}

bool Desert::set_feature(const Feature &feat) {
    this->feat = feat;
    return true;
}

Resource& Desert::get_rand_resource(const Resource::ResourceType type,
        int rnum) const {
    switch (type) {
        case Resource::Bonus:
