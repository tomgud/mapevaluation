...
private:
	...
	double fitness[5];
public:
	IndGrade(unsigned int id, double fit1, double fit2, double fit3, double fit4, double fit5);
	IndGrade& operator=(const IndGrade&);
	void set_selected(unsigned int id) { this->selected = (id < 5) ? id : this->selected; }
	...
	double get_sum() const { return (fitness[0] + fitness[1] + fitness[2] + fitness[3] + fitness[4]); }
	bool operator>(const IndGrade& ref) { return this->get_sum() > ref.get_sum(); }
};
