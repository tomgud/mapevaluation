...
Civ5Map& Civ5Map::operator=(const Civ5Map& c) {
    ...
    // TODO add the vectors correctly
...
void Civ5Map::init_map() {
	if (this->map_height != 0 && this->map_width != 0) {
		this->map = (Tile***)calloc(this->map_height, sizeof(Tile**));
		for (int h = 0; h < this->map_height; ++h) {
			this->map[h] = (Tile**)calloc(this->map_width, sizeof(Tile*));
		}
	}
	this->init = true;
}
...
void Civ5Map::to_map(Map& container) {
	if (this->map_width == Map::_DIMX && this->map_height == Map::_DIMY) {
		for(int i = 0; i < this->map_height; ++i) {
			for (int j = 0; j < this->map_width; ++j) {
				container.set_at(j, this->map_height-1-i, this->map[i][j]);
			}
		}
	} else {
		// Error
...
unsigned int Civ5Map::get_terrain_types_length() const { return this->terrain_types.size(); }
unsigned int Civ5Map::get_feature_types_length() const { return this->feature_types.size(); }
unsigned int Civ5Map::get_resource_types_length() const { return this->resource_types.size(); }
