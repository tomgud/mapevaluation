...
int main(int argc, char** argv) {
    ...
    if (evaluate && use_mapfile) {
        ...
        boost::mt19937 rng(seed);
        boost::random::uniform_int_distribution<> y_gen(0, print_out_map->get_dimy()-1);
        boost::random::uniform_int_distribution<> x_gen(0, print_out_map->get_dimx()-1);
        ...
        for (unsigned int i = 0; i < evaluate_iterations; ++i) {
            for (int j = 0; j < print_out_pm.get_no_of_players(); ++j) {
                ...
                cur_pl_st = print_out_map->search_circular([&] (std::shared_ptr<Tile> t) {
        ...
        for (unsigned int i = 0; i < genomes.size(); ++i) {
            ...
            links << "<td>" << Fitness::throughput_per_player(*print_out_map, print_out_pm) << "</td>";
            links << "<td>" << Fitness::strategic_resource(*print_out_map, print_out_pm) << "</td>";
            links << "<td>" << Fitness::unique_resources(*print_out_map, print_out_pm) << "</td>";
            links << "<td>" << Fitness::starting_positions(*print_out_map, print_out_pm) << "</td>";
            links << "<td>" << Fitness::players_in_a_continent(*print_out_map, print_out_pm) << "</td>";
            /// Old setup
            // links << "<td>" << Fitness::players_in_a_continent(*print_out_map, print_out_pm) << "</td>";
            // links << "<td>" << Fitness::starting_positions(*print_out_map, print_out_pm) << "</td>";
            // links << "<td>" << Fitness::unique_resources(*print_out_map, print_out_pm) << "</td>";
        ...
        Genome* g = new Genome(10, 20, 4);
        PlayerManager* pm = new PlayerManager(*g);
        //print_help();
    }
