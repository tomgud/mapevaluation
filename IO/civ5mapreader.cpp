...
Civ5MapReader::Civ5MapReader()
    : map(), filename("") {}

Civ5MapReader::Civ5MapReader(string filename)
    : map(), filename(filename) {}

void Civ5MapReader::report_error(int bytes_read, int pos) {
    // Do some crazy stuff.
    cout << "FAILURE | At position :" << pos << " after reading ";
...
void Civ5MapReader::read() {
    ...
    // Reading map width
    if (!map_file.read(reinterpret_cast<char*>(buffer), 4)) {
        this->report_error(map_file.gcount(), map_file.tellg());
    }
    ...
    // Terrain list
    byte str_buffer[512];
    if (!map_file.read(reinterpret_cast<char*>(str_buffer), 100)) {
        this->report_error(map_file.gcount(), map_file.tellg());
    }
    str_val = "";
    for (int i = 0; i < 100; ++i) {
    ...
    // Feature type list
    if (!map_file.read(reinterpret_cast<char*>(str_buffer), 280)) {
        this->report_error(map_file.gcount(), map_file.tellg());
    }
    str_val = "";
    for (int i = 0; i < 280; ++i) {
    ...
    // Resource list
    if (!map_file.read(reinterpret_cast<char*>(str_buffer), 417)) {
        this->report_error(map_file.gcount(), map_file.tellg());
    }
    str_val = "";
    for (int i = 0; i < 417; ++i) {
    ...
    if (!map_file.read(reinterpret_cast<char*>(buffer), 2)) {
        this->report_error(map_file.gcount(), map_file.tellg());
    }
    ...
    i_val = this->get_int(buffer, 4);
    // cout << map_file.tellg() << " String3 Map description ? ";
    // cout << i_val << endl;

    // String3 - Some map description
    // Using the newest value in i_val to determine how long it should be.
    if (!map_file.read(reinterpret_cast<char*>(str_buffer), i_val)) {
        this->report_error(map_file.gcount(), map_file.tellg());
    }
    string map_desc("");
    ...
    for (int i = 0; i < height; ++i) {
        for (int j = 0; j < width; ++j) {
            if (!map_file.read(reinterpret_cast<char*>(tile_buffer), 8)) {
                this->report_error(map_file.gcount(), map_file.tellg());
            }
            if (tile_buffer[0] != 0xFF) {
                // (int)tile_buffer[0] is the tile / terrain id
                str_val = this->map.get_terrain_types(
                        static_cast<int>(tile_buffer[0]));
                if (str_val == "TERRAIN_DESERT") {
                    ...
                    cout << "ERROR Not recognized terrain type ";
                    cout << str_val <<  endl;
                }
            }
            ...
            if (tile_buffer[1] != 0xFF) {
                // Resource id
                str_val = this->map.get_resource_types(
                        static_cast<int>(tile_buffer[1]));
                cur_tile->set_resource(*(new Resource(str_val)));
                // cout << j << "," << height-1-i << ": " <<  str_val << " : ";
                // cout << cur_tile->get_resource().get_string() << "<br />";
            }
            if (tile_buffer[2] != 0xFF) {
                // First feature type id
                // (int)tile_buffer[2]
                str_val = this->map.get_feature_types(
                        static_cast<int>(tile_buffer[2]));
                cur_tile->set_feature(*(new Feature(str_val)));
                // cout << j << "," << i << ": " <<  str_val << " : ";
                // cout cur_tile->get_feature().get_string() << "<br />";
            }
            if (tile_buffer[3] != 0xFF) {
                // River.
                // cout << map_file.tellg() << "|" << j << ","
                // cout i << ": " << (int)(tile_buffer[3]) << "<br />";
            }
            ...
            if (tile_buffer[6] != 0xFF) {
                ...
                // cout << i << "," << j << ": " << this->map.get_feature_types
                // ((int)tile_buffer[6]+8) << "<br />";
                str_val = this->map.get_feature_types(
                        static_cast<int>(tile_buffer[6])+8);
                cur_tile->set_feature(*(new Feature(str_val)));
                // cout << j << "," << i << ": " <<  str_val << " : "
                // << cur_tile->get_feature().get_string() << "<br />";
            }
