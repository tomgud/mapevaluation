...
// TODO(tomgud): Am I changing anything in the Tiles and the map?
// if not, change this to const references instead of pointers.
AStar::AStar(const Tile& start, const Tile& end, const Map* m)
    : finished(false), reached_target(false),
...
bool AStar::search() {
    ...
    push_heap(this->next_tiles.begin(), this->next_tiles.end(),
            SearchTileComparator() );
    // Starting the search.
    int count(0);
    while (!this->next_tiles.empty()) {
        ...
        pop_heap(this->next_tiles.begin(), this->next_tiles.end(),
                SearchTileComparator() );
...
void AStar::populate_queue(SearchTile* st) {
    ...
    for (unsigned int i = 0; i < nei.size(); ++i) {
        ...
        // if we reach here we can start to
        // think about looking at this successor
        c_gc = st->get_gcost() + this->map->at(nei[i])->get_movement_cost();

        // First linear search of open list to find node
        typename vector<SearchTile * >::iterator open_st_result;
        for (open_st_result = this->next_tiles.begin();
                open_st_result != this->next_tiles.end(); open_st_result++) {
        ...
        if (open_st_result != this->next_tiles.end()) {
            // We found a tile in next_tiles that
            // is the same one we are looking at
        ...
        for (closed_st_result = this->visited_tiles.begin();
                closed_st_result != this->visited_tiles.end();
                closed_st_result++) {
            // If we found the same state already in our next tiles (open list)
        ...
        if (open_st_result != this->next_tiles.end()) {
            ...
            make_heap(this->next_tiles.begin(),
                    this->next_tiles.end(), SearchTileComparator());
        }
        ...
        push_heap(this->next_tiles.begin(),
                this->next_tiles.end(), SearchTileComparator());
    }
